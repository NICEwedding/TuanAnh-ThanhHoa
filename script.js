...
const API_URL = "https://script.google.com/macros/s/AKfycbwSa0p2g1KTbRy3xv9DHnlL8zkKj0ZXXKrTxHpMm1yrFi1Bf120eix4tm74vpSRmLqxPw/exec";

/* ==================================
   LOAD DATA – CHẠY VÒNG LẶP VÔ TẬN
================================== */
function loadWishes() {
  fetch(API_URL)
    .then(r => r.json())
    .then(json => {
      ...
      // Tự tìm tên cột (đỡ bị sai key)
      let html = "";
      data.forEach(item => {
        const ks = Object.keys(item);
        const nameField = ks.find(k => k.toLowerCase().includes("tên")) || ks[0];
        const wishField = ks.find(k => k.toLowerCase().includes("chúc")) || ks[1];

        html += `
          <div class="wish-item">
            <b>${item[nameField] || ""}:</b> ${item[wishField] || ""}
          </div>`;
      });

      // Nhân đôi để auto-loop
      inner.innerHTML = html + html;

      // Nếu ít hơn 6 dòng → khỏi chạy
      if (data.length < 1) {
      ...
      // Tốc độ
      const secondsPerItem = 2.200;
      inner.style.animationDuration = (data.length * secondsPerItem) + "s";

      // Restart animation
      inner.classList.remove("auto-scroll");
      void inner.offsetWidth;
      inner.classList.add("auto-scroll");
    })
...
/* ==================================
   GỬI LỜI CHÚC – BẢN FIX CHUẨN
================================== */
document.getElementById("send-wish").onclick = () => {

  const name = document.getElementById("in-name").value.trim();
  const confirm = document.getElementById("in-confirm").value;
  const msg = document.getElementById("in-msg").value.trim();

  if (!name || !msg) {
    alert("Nhập tên và lời chúc nhé!");
    return;
  }

  let done = false; // CHẶN GỌI LẠI 2 LẦN

  // Auto hoàn tất sau 0.8s nếu API chậm
  ...
  fetch(API_URL, {
    ...
    .then(res => {
      ...
      clearTimeout(timeout);
      finishFast(true); // thành công
    })
    .catch(() => {
      clearTimeout(timeout);
      finishFast(); // lỗi mạng → vẫn đóng popup ngay
    });
  ...
  /* ====================================
       HÀM CHỈ CHẠY 1 LẦN
  ==================================== */
  function finishFast(success = false) {
    if (done) return;  // chặn double chạy
    done = true;

    if (success) loadWishes();
    ...
    /* ===============================
       TẮT POPUP NGAY – KHÔNG DELAY
    =============================== */
    popupForm.style.transition = "none";
    popupForm.style.opacity = "0";
    popupForm.style.display = "none";

    /* ===============================
       HIỆN POPUP CẢM ƠN NGAY – 1 LẦN
    =============================== */
    popupThanks.style.transition = "none";
